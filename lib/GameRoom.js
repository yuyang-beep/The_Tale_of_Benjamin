...
class Player {
  constructor(id, name) {
    ...
    this.connected = true;
    this.roundCoinsBreakdown = [null, null, null, null, null];
  }
...
class GameRoom {
  ...
  submitAction(playerId, action) {
    ...
    const MAXCOUNTS = { 10: 2, 5: 4, 1: 10 };

    if (gr === 1) {
      if (action.type !== 'initial') return { error: 'round 1 requires initial placement' };
      const err = p.pool.setInitial(action.investObj ?? {});
      if (err) return { error: err };
      p.roundCoins[personalIdx] = p.pool.investedTotal;
      p.roundCoinsBreakdown[personalIdx] = { ...p.pool.invested };
    } else {
      if (action.type !== 'full') return { error: 'round 2+ requires type:full' };

      // Step 1: fresh allocation for current round
      const err = p.pool.setInitial(action.investObj ?? {});
      if (err) return { error: err };
      p.roundCoins[personalIdx] = p.pool.investedTotal;
      p.roundCoinsBreakdown[personalIdx] = { ...p.pool.invested };

      // Step 2: optional ±1 adjustment of previous personal round
      const prevIdx = p.isBenjamin ? personalIdx + 1 : personalIdx - 1;
      const prevAdj = action.prevAdjust ?? { type: 'pass' };
      if (prevAdj.type !== 'pass' && prevIdx >= 0 && prevIdx < 5 && p.roundCoinsBreakdown[prevIdx]) {
        const prevBrk = { ...p.roundCoinsBreakdown[prevIdx] };
        const d = parseInt(prevAdj.denom);
        if (!MAXCOUNTS[d]) return { error: 'invalid denomination' };
        if (prevAdj.type === 'add') {
          if ((prevBrk[d] ?? 0) >= MAXCOUNTS[d]) return { error: `already at max ${d}-coins in previous round` };
          prevBrk[d] = (prevBrk[d] ?? 0) + 1;
        } else if (prevAdj.type === 'remove') {
          if ((prevBrk[d] ?? 0) <= 0) return { error: `no ${d}-coin in previous round to remove` };
          prevBrk[d]--;
        }
        p.roundCoinsBreakdown[prevIdx] = prevBrk;
        p.roundCoins[prevIdx] = [10, 5, 1].reduce((s, d2) => s + d2 * (prevBrk[d2] ?? 0), 0);
      }
    }
  ...
  publicState(viewerId) {
    ...
    const playersPublic = [...this.players.values()].map(p => {
      ...
      return {
        ...
        roundCoinsBreakdown: isViewer ? [...p.roundCoinsBreakdown] : undefined,
        hasActed: p.hasActed,
