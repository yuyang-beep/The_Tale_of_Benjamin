...
class GameRoom {
  ...
  // Normal player guesses Benjamin's id; Benjamin guesses ranking array.
  submitGuess(playerId, guess) {
    ...
    if (!p.isBenjamin) {
      if (p.guessedBenjaminCorrectly) {
        p.hasGuessed = true; this.guessesDone.add(playerId); return { ok: true };
      }
  ...
  publicState(viewerId) {
    ...
    // Post-adjust score reveal: sorted standings visible at start of invest phase
    ...
    if (inInvest && gr >= 2) {
      adjustReveal = [...this.players.values()]
        ...
        .map(p => ({
          name: p.name,
          totalScore: gr >= 3 ? p.totalScore : (p.id === viewerId ? p.totalScore : null),
        }));
    }
    ...
    return {
      ...
      leaderboard,
      adjustReveal,   // non-null during invest sub-phase (round 2+)
